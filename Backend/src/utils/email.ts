...
export const send2FAOTP = async ( to: string, otp: string, fullName: string ): Promise<void> => {
    await getTransporter().sendMail({
        from: `"Dhuno 🎧" <${process.env.SMTP_USER}>`,
        to,
        subject: "Your Dhuno Verification Code",
        html: `
            <div style="font-family:'Segoe UI',sans-serif;background:#0f131b;color:#fff;padding:40px;border-radius:16px;max-width:480px;margin:0 auto;">
                <h1 style="color:#5affda;margin:0 0 4px;">🎧 Dhuno</h1>
                <p style="color:#64748b;margin:0 0 32px;font-size:13px;">Security Verification</p>

                <h2 style="margin:0 0 16px;">Verification Code</h2>
                <p style="color:#cbd5e1;">Hi <strong>${fullName}</strong>,</p>
                <p style="color:#cbd5e1;">Use the code below to complete your login. It expires in <strong>10 minutes</strong>.</p>

                <div style="background:#1c2027;border:1px solid rgba(90,255,218,0.15);border-radius:12px;padding:28px;text-align:center;margin:28px 0;">
                    <span style="font-size:42px;font-weight:900;letter-spacing:14px;color:#5affda;">${otp}</span>
                </div>

                <p style="color:#64748b;font-size:13px;">If you didn't attempt to log in, please secure your account immediately.</p>
...
export const sendPasswordChangedEmail = async ( to: string, fullName: string ): Promise<void> => {
    await getTransporter().sendMail({
        from: `"Dhuno 🎧" <${process.env.SMTP_USER}>`,
        to,
        subject: "Your Dhuno Password was Changed",
        html: `
            <div style="font-family:'Segoe UI',sans-serif;background:#0f131b;color:#fff;padding:40px;border-radius:16px;max-width:480px;margin:0 auto;">
                <h1 style="color:#5affda;margin:0 0 4px;">🎧 Dhuno</h1>
                <p style="color:#64748b;margin:0 0 32px;font-size:13px;">Security Notification</p>

                <h2 style="margin:0 0 16px;">Password Changed Successfully</h2>
                <p style="color:#cbd5e1;">Hi <strong>${fullName}</strong>,</p>
                <p style="color:#cbd5e1;">This is a confirmation that your Dhuno account password has been successfully changed.</p>

                <div style="background:#1c2027;border:1px solid rgba(90,255,218,0.15);border-radius:12px;padding:28px;text-align:center;margin:28px 0;">
                    <span style="font-size:24px;font-weight:700;color:#5affda;">Security Confirmed</span>
                </div>

                <p style="color:#64748b;font-size:13px;">If you did not make this change, please contact our support team immediately or use the forgot password feature to reset it.</p>
